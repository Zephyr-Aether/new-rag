...
import { Bot, ChevronDown, Paperclip, Send, Square, UserRound } from 'lucide-react'
...
const MessageBubble = memo(function MessageBubble({ m, openTools, openCitations, onToggleTools, onToggleCitations, onRegenerate, onDeleteMessage }: {
  m: ChatMsg
  openTools: boolean
...
const SessionItemView = memo(function SessionItemView({ s, active, onOpen, onRename, onDelete }: {
  s: SessionItem
  active: boolean
  onOpen: () => void
  onRename: () => void
...
export default function Chat() {
  ...
  async function uploadFile(f: File) {
    ...
    if (f.size > MAX_UPLOAD_BYTES) {
      window.alert(`文件超过 ${(MAX_UPLOAD_BYTES / 1024 / 1024).toFixed(0)}MB 上限`)
      return
    }
    setUploading(true)
    try {
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      ...
      <PageHeader
        title="对话"
        desc="先发一个问题看它怎么答，需要资料时上传文档，再回来验证引用和工具链路。"
        actions={
          <>
