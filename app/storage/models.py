...
class MessageRow(Base):
    ...
    content: Mapped[str] = mapped_column(Text)
    state: Mapped[str | None] = mapped_column(String(32), nullable=True)  # run 终态（中断/失败可回溯）
...
class AgentRunRow(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "agent_id", "run_id", name="uq_run_scope"),
        # §32.5 幂等：客户端 key（重试不重复建 run）；多 NULL 允许
        UniqueConstraint("tenant_id", "client_run_id", name="uq_run_client_id"),
    )
